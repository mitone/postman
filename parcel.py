        ...
        if "parcel_id" in data:
            ...
            if "parcel_destination_msg_id" in data:
                ...
                text = md.text(
                    ...
                    md.underline(data["country_src"]),
                    md.bold("в"),
                    md.underline(data["country_dst"]),
                )
                ...
                sender_by_countries = await search_sender(query.message, data["parcel_id"], by_countries=True)
                if not sender_by_countries:
                    await bot.edit_message_text(
                        texts.PARCEL_SEARCH_BY_COUNTRIES_TEXT,
                        state.chat,
                        data["last_msg_id"]
                    )

                del data["country_src"]
                del data["country_dst"]
...
async def parcel_city_dst_selected(message, state, city_id, city_name, edit=False):
    ...
    if parcel_id is not None:
        ...
        if msg:
            async with state.proxy() as data:
                country_src = data['city_src'].split('/')[0]
                country_dst = data['city_dst'].split('/')[0]

                data["country_src"] = country_src
                data["country_dst"] = country_dst
...
async def process_parcel_type(query: CallbackQuery, state: FSMContext):
    ...
    if is_ready:
        ...
        if parcel_id is None:
            # it's a new parcel
            parcel_type = name.lower()

            text = md.text(md.bold("Тип посылки: "), parcel_type)
            ...
            last_msg_id = query.message
            if not senders:
                last_msg_id = await query.message.answer(
            ...
            async with state.proxy() as data:
                ...
                data["last_msg_id"] = last_msg_id.message_id
